...
class SDPA(nn.Module):
    ...
    def backward_ref(self,
        ...
        return (q_grad, k_grad, v_grad, p_grad, s2_grad, s_grad)
...
def test_sdpa(dtype,
    ...
    q_grad, k_grad, v_grad, p_grad, s2_grad, s_grad = test_model.backward_ref(grad)
    dump_dict = {}
    ...
    if (dropout_p > 0):
        set_dict(dump_dict, 'p2', GRAD_DICT['p2'])
    set_dict(dump_dict, 'p_grad', p_grad)
    set_dict(dump_dict, 's2_grad', s2_grad)
    ...
    print('k_grad ', is_close(k.grad, k_grad), is_close(k.grad, k2.grad), is_close(k_grad, k2.grad))
    print('q_grad ', is_close(q.grad, q_grad), is_close(q.grad, q2.grad), is_close(q_grad, q2.grad))
    print('v_grad ', is_close(v.grad, v_grad), is_close(v.grad, v2.grad), is_close(v_grad, v2.grad))
...
if __name__ == '__main__':
    ...
    # loop_run()
    # test_sdpa(torch.bfloat16, 123, 4, 4, 2, 512, 512, 128, 64, 0.3, is_causal=True)
    # GRAD_DICT = {}
    test_sdpa(torch.bfloat16, 123, 4, 4, 4, 512, 512, 128, 64, 0.3, is_causal=True)
    GRAD_DICT = {}
    test_sdpa(torch.bfloat16, 123, 4, 4, 4, 512, 512, 128, 64, is_causal=True)
    GRAD_DICT = {}
    test_sdpa(torch.bfloat16, 123, 4, 4, 4, 512, 512, 128, 64, 0.3, is_causal=False)
    GRAD_DICT = {}
