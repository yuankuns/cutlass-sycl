...
struct ScaleExpHelper {
    ...
    static CUTLASS_DEVICE void apply(
        Tensor<Engine0, Layout0> &tensor, float scale, float neg_max_scaled, bool debug = false) {
...
struct ScaleExpHelper<Int<8>> {
    ...
    static CUTLASS_DEVICE void apply(
        Tensor<Engine0, Layout0> &tensor, float scale, float neg_max_scaled, bool debug = false) {
        // ============================================================================
        // WORKING VERSION: Process 8 elements one at a time to avoid GRF aliasing issues
        ...
        */
        // ============================================================================
        // ORIGINAL VERSION (BROKEN for headdim=192 + seq_len=523):
        // Uses intel::float8 recast which fails due to compiler optimization issues.
        // headdim=256 works, headdim=192 produces NaN in dq output.
        // Adding debug print statements changes compiler behavior and makes it work.
        // ============================================================================
        intel::float8& vals = *recast_ptr<intel::float8>(&tensor(0));
        __asm__ volatile (
            "{\n"
            ".decl VALS v_type=G type=F num_elts=128 alias=<%0,0>\n"
            ".decl SCALE v_type=G type=F num_elts=16 alias=<%1,0>\n"
            ".decl NEG_MAX v_type=G type=F num_elts=16 alias=<%2,0>\n"
            "mad (M1, 32) VALS(0,0)<1>  VALS(0,0)<1;1,0>  SCALE(0,0)<0;1,0> NEG_MAX(0,0)<1;1,0>\n"
            "mad (M1, 32) VALS(0,32)<1> VALS(0,32)<1;1,0> SCALE(0,0)<0;1,0> NEG_MAX(0,0)<1;1,0>\n"
            "mad (M1, 32) VALS(0,64)<1> VALS(0,64)<1;1,0> SCALE(0,0)<0;1,0> NEG_MAX(0,0)<1;1,0>\n"
            "mad (M1, 32) VALS(0,96)<1> VALS(0,96)<1;1,0> SCALE(0,0)<0;1,0> NEG_MAX(0,0)<1;1,0>\n"
            "exp (M1, 32) VALS(0,0)<1>  VALS(0,0)<1;1,0>\n"
            "exp (M1, 32) VALS(0,32)<1> VALS(0,32)<1;1,0>\n"
            "exp (M1, 32) VALS(0,64)<1> VALS(0,64)<1;1,0>\n"
            "exp (M1, 32) VALS(0,96)<1> VALS(0,96)<1;1,0>\n"
            "}\n"
            : "+rw"(vals)
            : "rw"(scale), "rw"(neg_max_scaled)
        );
