...
struct FAKernel {
    ...
    */
    ...
    using _K = Int<MMA_Atom_arch::K * 2>;
    using SubgroupLayoutSdP = Layout<Shape<_1, _8, _1>,
                                     Stride<_8, _1, _0>>;
    using SubgroupLayoutdKV = Layout<Shape<_8, _1, _1>,
                                     Stride<_1, _1, _0>>;
    using TileShapeSdP = Layout<Shape<Int<kBlockM>, Int<kBlockN>, _64>>;
    using TileShapedKV = Layout<Shape<Int<kBlockN>, Int<kHeadDim>, Int<kBlockM>>>;
         ...
         int wg_n, bool debug = false)
{
  ...
  for (int k_tile = 0; k_tile < k_tile_count; k_tile++, k_tile_prefetch++) {
    ...
    /* Shuffle data from copy fragments to MMA fragments */
    reorder(tArA, tCrA);
    reorder(tBrB, tCrB); // reorder works here
...
int main(int argc, char**argv) {
    ...
    random_init(123 + 1, q_h.data(), q_h.size());
    random_init(123 + 2, k_h.data(), k_h.size());
    random_init(123 + 3, v_h.data(), v_h.size());
    random_init(123 + 4, do_h.data(), do_h.size());

    std::vector<T> dk_h(BATCH * NUM_HEAD_KV * SEQ_LEN_KV * HEAD_SIZE_QK);
    std::vector<T> dv_h(BATCH * NUM_HEAD_KV * SEQ_LEN_KV * HEAD_SIZE_VO);
    std::vector<T> dp_h(BATCH * NUM_HEAD_Q * SEQ_LEN_QO * SEQ_LEN_KV);
    std::vector<T> p_h(BATCH * NUM_HEAD_Q * SEQ_LEN_QO * SEQ_LEN_KV);
    ...
    // alloc qkv
    T *q_d = compat::malloc<T>(q_h.size());
    T *k_d = compat::malloc<T>(k_h.size());
    T *v_d = compat::malloc<T>(v_h.size());

    // alloc ps
    T *p_d = compat::malloc<T>(BATCH * NUM_HEAD_Q * SEQ_LEN_QO_PAD * SEQ_LEN_KV_PAD);
    T *p_d_ref = compat::malloc<T>(BATCH * NUM_HEAD_Q * SEQ_LEN_QO_PAD * SEQ_LEN_KV_PAD);
    T *s_d = compat::malloc<T>(BATCH * NUM_HEAD_Q * SEQ_LEN_QO_PAD * SEQ_LEN_KV_PAD);

    // alloc grad output
    T *do_d = compat::malloc<T>(do_h.size());

    // alloc grad test on device
    V *dqaccum_d = compat::malloc<V>(BATCH * NUM_HEAD_Q * SEQ_LEN_QO_PAD * HEAD_SIZE_QK);
    T *dk_d = compat::malloc<T>(dk_h.size());
    T *dv_d = compat::malloc<T>(dv_h.size());
    T *dv_d_ref = compat::malloc<T>(dv_h.size());
    ...
    verify_with_cutlass(q_d, k_d, p_d_ref,
                        SEQ_LEN_QO, SEQ_LEN_KV, HEAD_SIZE_QK, BATCH * NUM_HEAD_Q, 1.0, 0.0, 'N', 'T');
    verify_with_cutlass(p_d_ref, do_d, dv_d_ref,
                        SEQ_LEN_KV, HEAD_SIZE_VO, SEQ_LEN_QO, BATCH * NUM_HEAD_Q, 1.0, 0.0, 'T', 'N');
    ...
    std::vector<T>  p_refe(BATCH * NUM_HEAD_Q * SEQ_LEN_QO * SEQ_LEN_KV);
    compat::memcpy<T>(p_refe.data(), p_d_ref, p_refe.size());
    compat::wait_and_throw();
    printf("p val: ");
    verify(p_refe.data(), p_test.data(), BATCH * NUM_HEAD_Q, SEQ_LEN_QO, SEQ_LEN_QO_PAD, SEQ_LEN_KV, SEQ_LEN_KV_PAD, atol, rtol);

    std::vector<T> dv_test(BATCH * NUM_HEAD_Q * SEQ_LEN_KV * HEAD_SIZE_VO);
    compat::memcpy<T>(dv_test.data(), dv_d, dv_test.size());
    std::vector<T> dv_refe(BATCH * NUM_HEAD_Q * SEQ_LEN_KV * HEAD_SIZE_VO);
    compat::memcpy<T>(dv_refe.data(), dv_d_ref, dv_refe.size());
    compat::wait_and_throw();
    printf("dV val: ");
    verify(dv_refe.data(), dv_test.data(), BATCH * NUM_HEAD_Q, SEQ_LEN_KV, HEAD_SIZE_VO, atol, rtol);
}
