...
class SDPA(nn.Module):
    ...
    def backward_ref(self,
        ...
        k_grad_red = num_head_reduce(k_grad, self.k)
        v_grad_red = num_head_reduce(v_grad, self.v)
        return (q_grad, k_grad, k_grad_red, v_grad, v_grad_red, p_grad, s_grad)
...
def test_sdpa(dtype,
    ...
    q_grad, k_grad, k_grad_red, v_grad, v_grad_red, p_grad, s_grad = test_model.backward_ref(grad)
    dump_dict = {}
    ...
    print('k_grad ', is_close(k_grad_red, k2.grad))
    print('q_grad ', is_close(q_grad, q2.grad))
    print('v_grad ', is_close(v_grad_red, v2.grad))
    if is_bhsd:
...
def loop_run():
    ...
    for h in [2, 1]:
        ...
        for seq_q in [512, 513, 523, 528, 543]:
            for seq_k in [512, 513, 523, 528, 543]:
...
if __name__ == '__main__':
    ...
    loop_run()
    # test_sdpa(torch.float16, 123, 4, 4, 2, 514, 513, 128, 128, is_bhsd=True)
    # GRAD_DICT = {}
